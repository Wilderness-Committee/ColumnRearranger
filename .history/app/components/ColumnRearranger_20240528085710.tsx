  ...
  const handleFileUpload = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      Papa.parse(file, {
        ...
        dynamicTyping: true,
        complete: (result) => {
          const processedData = processColumnHeaders(result.data);
          setCsvData(processedData);
          rearrangeColumns(processedData);
        },
      });
    }
  };

  const processColumnHeaders = (data: any[]) => {
    return data.map((row) => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-4">
      <div className="bg-white shadow-md rounded p-6 w-full max-w-lg">
        <h2 className="text-2xl font-semibold mb-4 text-gray-800">Column Rearranger Tool</h2>
        <input
          type="file"
          accept=".csv"
                ...
                <thead>
                  <tr>
                    {Object.keys(rearrangedData[0] || {}).map((col) => (
                      <th key={col} className="py-2 px-4 border-b text-gray-800">{col}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {rearrangedData.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      {Object.values(row).map((cell, cellIndex) => (
                        <td key={cellIndex} className="py-2 px-4 border-b text-gray-800">
